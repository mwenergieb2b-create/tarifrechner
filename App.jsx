...
// Erkennt offensichtliche Fantasienummern, die technisch durch ein reines
// Format-Regex rutschen würden (z. B. "0000000000", "1111111111",
// "123456789", "987654321").
function isFantasyPhonePattern(digitsOnly) {
  if (/^(\d)\1+$/.test(digitsOnly)) return true; // alle Ziffern identisch
  const ascending = "0123456789";
  const descending = "9876543210";
  if (digitsOnly.length >= 6 && (ascending.includes(digitsOnly) || descending.includes(digitsOnly))) {
    return true; // einfache auf-/absteigende Zahlenfolge
  }
  return false;
}

// +49/0049 auf nationales Format (führende 0) normalisieren, um die
// deutschen Mobilfunk-Vorwahlen zu prüfen.
...
const validators = {
  ...
  telefon: (v) => {
    ...
    // Nur echte deutsche Mobilfunk-Vorwahlen zulassen (015x/016x/017x) —
    // schließt sowohl Fantasienummern als auch Festnetznummern aus.
    const national = toNationalFormat(cleaned);
    if (!/^0(15|16|17)\d{7,9}$/.test(national)) return false;
    // Zusätzliche Prüfung gegen den echten Rufnummernplan — kostenlos,
    // läuft komplett im Browser, kein externer API-Call, kein SMS-Versand.
    try {
...
/* ---------------------------------------------------------
   n8n-Anbindung
--------------------------------------------------------- */
// TODO Max: Fortschritts-Webhook eintragen, sobald es dafür einen Endpunkt gibt
const N8N_PROGRESS_URL = "https://kwh-beratung.app.n8n.cloud/webhook/HIER-PROGRESS-WEBHOOK-EINTRAGEN";
// Submit geht jetzt direkt an unsere eigene Vercel-Function, die Pipedrive
// per API befüllt (kein n8n mehr dazwischen).
...
export default function TarifRechner() {
  ...
  useEffect(() => {
    try {
      ...
      // Nur überspringen, wenn die per Link mitgegebene Nummer auch
      // WIRKLICH gültig ist — sonst würde eine kaputte/Platzhalter-Nummer
      // im Link ungeprüft durchrutschen und das Feld bliebe unsichtbar.
