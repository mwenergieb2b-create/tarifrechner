...
export default function TarifRechner() {
  ...
  const fraction = step === "intro" ? 0 : step === "abschluss" ? 1 : (stepIndex + 1) / (activeSteps.length + 1);
  ...
  async function next() {
    if (step === "intro") return goTo(activeSteps[0]);
  ...
  function back() {
    if (step === activeSteps[0]) return goTo("intro");
    const i = activeSteps.indexOf(step);
    if (i <= 0) return;
    ...
    <div className="min-h-screen w-full" style={{ background: c.bg, fontFamily: fontBody }}>
      ...
      <div className="max-w-md mx-auto px-5 py-4 sm:py-6">
        ...
        >
          ...
          {step === "intro" && (
            <div className="step-enter">
              <Badge tone="green">Kostenlose Tarifprüfung · 2 Minuten</Badge>
              <h1 className="text-[27px] sm:text-3xl leading-[1.13] mt-4 mb-3 font-extrabold" style={{ fontFamily: fontDisplay, color: c.ink }}>
                Wie viel können Sie bei Ihrem Stromtarif sparen?
              </h1>
              <p className="text-[15px] leading-relaxed mb-6" style={{ color: c.inkSoft }}>
                Wir prüfen Ihre Wechseloptionen — auch wenn andere Anbieter Sie wegen Ihrer Bonität bereits abgelehnt haben.
              </p>
              <div className="space-y-3 mb-8">
                {[
                  ["1", "Ein paar Angaben zu Verbrauch & Wohnort"],
                  ["2", "Ihren passenden Tarif mit Preis ansehen"],
                  ["3", "Kündigung & Anmeldung übernehmen wir für Sie"],
                ].map(([n, txt]) => (
                  <div key={n} className="flex items-center gap-3">
                    <div className="flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center text-xs font-extrabold" style={{ background: c.blueTint, color: c.blue, fontFamily: fontDisplay }}>
                      {n}
                    </div>
              ...
              >
                Jetzt Tarif prüfen <ArrowRight size={16} />
              </button>
              <p className="text-xs text-center mt-3" style={{ color: c.stone }}>
                Keine Verpflichtung. Sie entscheiden am Ende, ob Sie wechseln.
              </p>
            </div>
          )}
          ...
          {step === "empfehlung" && (
            ...
            >
              ...
              {!istIndividuell && (
                <p className="text-xs leading-relaxed" style={{ color: c.stone }}>
                  Beispielhafte Berechnung auf Basis Ihres Jahresverbrauchs. Der endgültige Preis wird Ihnen vor Vertragsabschluss transparent bestätigt. Sie gehen mit der Anfrage noch keine Verpflichtung ein.
                </p>
              )}
            </StepShell>
          )}
